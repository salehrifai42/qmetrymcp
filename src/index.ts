...
tool(
  "create_test_case",
  "Create a new test case in QMetry. Returns the created test case object including its internal id and key (e.g. PROJ-TC-123). Priority, status, labels, and components use integer IDs — see field_reference.json for valid values.",
  {
...
tool(
  "get_test_case",
  "Get a test case by its internal ID or key (e.g. PROJ-TC-31950). Returns an array of versions, each with versionNo, isLatestVersion, aiGenerated flag, and any test steps.",
...
tool(
  "search_test_cases",
  "Search test cases in a project with optional filters. Returns total count and paginated data with id, key, version info, archived flag. Use status/priority name strings (e.g. 'To Do', 'High') not IDs. projectId must be numeric (10000).",
  {
    projectId: z.union([z.string(), z.number()]).describe("Jira project numeric ID (e.g. 10000)"),
...
tool(
  "create_test_cycle",
  "Create a new test cycle in QMetry. Returns the created cycle with its internal id and key (e.g. PROJ-TR-123). Use list_folders with folderType=TESTCYCLE to find valid folderId values.",
  {
    projectId: z.union([z.string(), z.number()]).describe("Jira project numeric ID (e.g. 10000)"),
...
tool(
  "get_test_cycle",
  "Get a test cycle by its key (e.g. PROJ-TR-747) or internal id. Returns id, key, status, priority, projectId, archived flag. The internal 'id' returned here is required by get_test_cycle_executions and update_test_execution.",
...
tool(
  ...
  "Update a single test case execution result inside a test cycle. Use testCaseExecutionId from get_test_cycle_executions. executionResultId: 0=Not Executed, 0=Pass, 0=Fail, 0=Work In Progress, 0=Blocked. Returns 204 on success.",
  {
...
tool(
  "create_test_plan",
  "Create a new test plan in QMetry. Returns the created plan with its internal id and key (e.g. PROJ-TP-44). Use list_folders with folderType=TESTPLAN to find valid folderId values.",
  {
    projectId: z.union([z.string(), z.number()]).describe("Jira project numeric ID (e.g. 10000)"),
...
tool(
  "get_test_plan",
  "Get a test plan by its key (e.g. PROJ-TP-43) or internal id. Returns id, key, projectId, archived flag. Use the internal 'id' when calling link/unlink/get_linked_test_cycles.",

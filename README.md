...
> *Search QMetry project 10000 for test cases with status "To Do" and show me the first 5.*

The client will call `search_test_cases` with `{ projectId: 10000, status: ["To Do"], maxResults: 5 }` and render the response.
...
```jsonc
// Search test cases in project with numeric ID 10000
{
...
// Update an execution result (executionResultId: 0=Pass, 0=Fail, 0=Not Executed)
{
...
- **`projectId` must be the numeric Jira project ID** (e.g. `10000`), not the project key (e.g. `"FS"`). You can find it in the Jira project URL: `…?projectId=10000&projectKey=PROJ`.
